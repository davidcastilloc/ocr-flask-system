...
def get_data(query)->list:
    ...
    def execute_query(inst, name, photo, data):
        try:
            inst.execute_query()
            data.append({
                'name': name,
                'photo': photo,
                'product': inst.get_result().to_dict()
            })
    ...
    execute_query(inst_bs, "BoticasYSalud", "BoticasYSalud.png", data)
    execute_query(inst_mf, "MiFarma", "Mifarma.png", data)
    execute_query(inst_bhs, "BoticasHogarYSalud", "BoticasHogarYSalud.png", data)
    execute_query(inst_ik, "InkaFarma", "Inkafarma.png", data)
    execute_query(inst_fu, "FarmaciaUniversal", "FarmaciaUniversal.png", data)
    execute_query(inst_bp, "BoticasPeru", "BoticasPeru.png", data)
